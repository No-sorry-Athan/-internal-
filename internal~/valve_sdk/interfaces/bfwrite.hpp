...
inline auto LoadLittleDWor(uint32* base, unsigned int dwordIndex)
{
	return LittleDWord(base[dwordIndex]);
}

inline void StoreLittleDWor(uint32* base, unsigned int dwordIndex, unsigned long dword)
{
...
inline void old_bf_write::WriteUBitLong(unsigned int curData, int numbits, bool bCheckRange)
{
	...
	if (nBitsWritten < nBitsLeft)
	{
		...
		dword = LoadLittleDWor((uint32*)m_pData, iDWord + 1);

		dword &= g_BitWriteMasks[0][nBitsLeft];
		dword |= curData;

		// write to stream (lsb to msb) properly 
		StoreLittleDWor((uint32*)m_pData, iDWord + 1, dword);
	}
...
inline unsigned int old_bf_read::ReadUBitLong(int numbits)
{
	...
	unsigned int dword1 = LoadLittleDWor((uint32*)m_pData, idword1);
	...
	else
	{
		...
		unsigned int dword2 = LoadLittleDWor((uint32*)m_pData, idword1 + 1);
