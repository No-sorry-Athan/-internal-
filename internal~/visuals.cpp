...
void Visuals::Player::RenderHealth()
{
	...
	Render::Get().RenderText(std::to_string(hp), x - 5, y + w + 1, 10.f, Color(red, green, 0, 255));
}
//--------------------------------------------------------------------------------
void Visuals::Player::RenderArmour()
{
	...
	//Render::Get().RenderText(std::to_string(armour), x + 3, y + w + 1, 10.f, Color(0, 50, 255, 255));
...
void Visuals::DrawBones(C_BasePlayer* pEntity)
{
	...
	for (int j = 0; j < studioHdr->numbones; j++) {
		mstudiobone_t* pBone = studioHdr->GetBone(j);
		if (pBone && (pBone->flags & BONE_USED_BY_HITBOX | BONE_USED_BY_ATTACHMENT) && (pBone->parent != -1)) {
			...
			int iChestBone = 6;
			Vector vBreastBone;
			Vector vUpperDirection = pEntity->GetBonePos(iChestBone + 1) - pEntity->GetBonePos(iChestBone);
			...
			g_VGuiSurface->DrawSetColor(g_Options.color_skeleton_clr);
			g_VGuiSurface->DrawLine(sParent.x, sParent.y, sChild.x, sChild.y);	
		}
	}
}
